...
export default function DashboardAbogada() {
  ...
  const createProcess = async () => {
    ...
    const { denounced, denouncer, province } = newProcess;
  
    if (!onlyLettersRegex.test(denounced)) {
      alert('El campo "Denunciado" solo debe contener letras');
      return;
    }
    if (!onlyLettersRegex.test(denouncer)) {
      alert('El campo "Denunciante" solo debe contener letras');
      return;
    }
    if (!onlyLettersRegex.test(province)) {
      alert('El campo "Provincia" solo debe contener letras');
      return;
    }
    try {
      const response = await fetch(`http://localhost:3000/api/processes`, {
        ...
        >
          {Object.keys(newProcess).map((field) => (
            <input
              ...
              onChange={(e) => setNewProcess({ ...newProcess, [field]: e.target.value })}
